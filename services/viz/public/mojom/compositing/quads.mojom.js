...
(function() {
  ...
  if (mojo.config.autoLoadMojomDeps) {
    mojo.internal.loadMojomIfNecessary(
        'services/viz/public/mojom/compositing/render_pass_id.mojom', 'render_pass_id.mojom.js');
  }
  ...
  RenderPassQuadState.prototype.initDefaults_ = function() {
    ...
    this.canUseBackdropFilterCache = false;
  ...
  RenderPassQuadState.validate = function(messageValidator, offset) {
    ...
    // validate RenderPassQuadState.renderPassId
  ...
  RenderPassQuadState.decode = function(decoder) {
    ...
    var version = decoder.readUint32();
    val.renderPassId =
        decoder.decodeStructPointer(render_pass_id$.RenderPassId);
    val.maskResourceId =
        decoder.decodeStruct(codec.Uint32);
    packed = decoder.readUint8();
    val.forceAntiAliasingOff = (packed >> 0) & 1 ? true : false;
    val.canUseBackdropFilterCache = (packed >> 1) & 1 ? true : false;
    decoder.skip(1);
    decoder.skip(1);
    decoder.skip(1);
    val.maskUvRect =
  ...
  RenderPassQuadState.encode = function(encoder, val) {
    ...
    encoder.writeUint32(0);
    encoder.encodeStructPointer(render_pass_id$.RenderPassId, val.renderPassId);
    encoder.encodeStruct(codec.Uint32, val.maskResourceId);
    packed = 0;
    packed |= (val.forceAntiAliasingOff & 1) << 0
    packed |= (val.canUseBackdropFilterCache & 1) << 1
